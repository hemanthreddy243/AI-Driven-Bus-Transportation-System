...
const StudentDashboard = () => {
  ...
  const [coming, setComing] = useState(null); // Default is null
  ...
  useEffect(() => {
    ...
    if (user) {
      ...
      const unsubscribe = onValue(studentRef, (snapshot) => {
        ...
        // If it's a new day, reset coming and update lastVoteDate
        if (!lastVoteDate || lastVoteDate !== today) {
          ...
          setComing(null); // Ensure local state reflects reset
        } else {
          setComing(data?.coming || null); // Use existing value if today
        }
      });
