...
const StudentDashboard = () => {
  ...
  const isVotingTime = () => {
    ...
    const canVote = (hours >= 11 && hours < 20) || (hours === 3 && minutes >= 30) || (hours === 4 && minutes <= 0);
    console.log(
  ...
  // Plot routes whenever busRoutes, showMyRouteOnly, studentInfo, or currentPage changes
  useEffect(() => {
    ...
    console.log("Current Page:", currentPage, "Total Display Routes:", displayRoutes.length);
    ...
    if (currentPage >= displayRoutes.length || currentPage < 0) {
      console.log("Current page is out of bounds:", currentPage, "Display Routes Length:", displayRoutes.length);
      setCurrentPage(0);
      return;
    }

    const [busId, route] = displayRoutes[currentPage];
    console.log("Selected Route for Bus", busId, ":", route);

    if (!route || !route.stops || route.stops.length === 0) {
      console.log("Selected route is empty for Bus", busId);
      return;
    }
    ...
    const polyline = new maps.Polyline({
      path,
      geodesic: true,
      strokeColor: route.color, // Use the color provided by the backend
      strokeOpacity: 1.0,
      strokeWeight: 2,
    ...
    route.stops.forEach((item) => {
      ...
      const marker = new maps.Marker({
        ...
        icon: stop.name === "College" ? {
          ...
          fillColor: route.color, // Use the bus color for stop markers
          fillOpacity: 1,
          strokeWeight: 0,
          scale: 8,
        },
  ...
  }, [busRoutes, showMyRouteOnly, studentInfo, currentPage]);
