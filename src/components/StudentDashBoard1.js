...
const StudentDashboard = () => {
  ...
  const isVotingTime = () => {
    ...
    // Allow voting at 09:00 AM to 09:15 AM UTC for testing, in addition to 11 AM - 8 PM UTC
    const canVote = (hours >= 11 && hours < 20) || (hours === 9 && minutes >= 0 && minutes <= 15);
    console.log(
  ...
  const handleComingResponse = async (response) => {
    if (!isVotingTime()) {
      alert("You can only respond between 11 AM and 8 PM.");
      return;
    }
    const user = auth.currentUser;
    if (user) {
              ...
              <div style={styles.votingBox}>
                ...
                {!isVotingTime() && (
                  <p style={styles.warning}>
                    Voting: 11 AM - 8 PM UTC (4:30 PM - 1:30 AM IST)
                  </p>
                )}
              </div>
